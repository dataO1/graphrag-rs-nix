...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # ── Query NPU: second OVMS on :9001 for low-latency queries ──
    systemd.services."graphrag-ovms-pull-query" = lib.mkIf cfg.queryNpu.enable {
      ...
      serviceConfig = {
        ...
        Environment = [
          ...
          "GRS_RERANKER_MODEL="
          "PYTHONPATH=${pullPyExtras}"
          "LD_LIBRARY_PATH=${lib.makeLibraryPath [ pkgs.stdenv.cc.cc.lib pkgs.zlib pkgs.libffi pkgs.openssl pkgs.glib ]}"
        ];
      };
    };

    virtualisation.oci-containers.containers."graphrag-ovms-query" = lib.mkIf cfg.queryNpu.enable {
      image = ovmsImage;
      autoStart = true;
    ...
    systemd.services."podman-graphrag-ovms-query".restartTriggers = lib.mkIf cfg.queryNpu.enable [
      pullScript
    ];
  };
}
